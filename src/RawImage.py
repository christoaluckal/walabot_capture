...
class RawImageApp(tk.Frame):
    ...
    def loop(self):
        ...
        with open('rawImage.pkl', 'wb') as f:
            pickle.dump(self.wlbt.images, f)
        
        with open('rawSignal.pkl', 'wb') as f:
            pickle.dump(self.wlbt.raw_signals, f)

        # print kb size of the file
        
        print(os.path.getsize('rawImage.pkl'))
        print(os.path.getsize('rawSignal.pkl'))
...
class Walabot:
    ...
    def triggerAndGetRawImageSlice(self):
        ...
        """
        self.wlbt.Trigger()
        image = self.wlbt.GetRawImageSlice()[0]

        import numpy as np

        np_im = np.array(image)

        ap = self.wlbt.GetAntennaPairs()[0]
        dims = self.getRawImageSliceDimensions()

        signal = self.wlbt.GetSignal(ap)

        self.images.append([image, dims])
...
def rawImage():
    ...
    try:
        ...
        ria = RawImageApp(root)
        ria.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
        root.geometry("+{}+{}".format(APP_X, APP_Y))  # set window location
        root.update()
        root.minsize(width=root.winfo_reqwidth(),
                     height=root.winfo_reqheight())
        root.mainloop()
    except Exception as e:
        images = ria.wlbt.images
        signals = ria.wlbt.raw_signals
